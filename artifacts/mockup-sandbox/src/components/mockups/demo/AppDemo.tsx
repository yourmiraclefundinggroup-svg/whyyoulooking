...
function loadArrayScripts(cb: () => void) {
  if (_scriptsReady) { cb(); return; }
  _readyListeners.push(cb);
  if (_scriptsInjected) return;
  _scriptsInjected = true;
  ARRAY_SCRIPTS.forEach(name => {
    if (document.querySelector(`script[data-array="${name}"]`)) {
      _settledCount++;
      if (_settledCount >= ARRAY_SCRIPTS.length) {
        _scriptsReady = true;
...
// ── ArrayWrapper — mirrors the real portal's cp-array-wrapper pattern ─────────
function ArrayWrapper({
  title, sub, badge, accentTop = false, ready, locked = false, children,
}: {
  title: string; sub?: string; badge?: React.ReactNode;
  accentTop?: boolean; ready: boolean; locked?: boolean; children: React.ReactNode;
}) {
  return (
      ...
      <div className="cp-array-wrapper-body">
        {locked ? (
          <div className="cp-array-locked">
            ...
            <div className="cp-array-locked-sub">This feature requires a subscription upgrade.</div>
          </div>
        ) : !ready ? (
...
type PageId =
  ...
  | "simulator" | "report" | "debt-analysis" | "debt-navigator" | "student-loan" | "subscription-manager"
...
const NAV_SECTIONS: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      {
        id: "score-tracker", label: "Score Tracker",
      ...
      },
      {
        ...
        icon: <Ico><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></Ico>,
      },
      {
        id: "pip-scan", label: "PIP Scan Results",
        icon: <Ico><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></Ico>,
      },
      {
        id: "credit-alerts", label: "Credit Alerts",
        icon: <Ico><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" /><path d="M13.73 21a2 2 0 0 1-3.46 0" /></Ico>,
      },
    ],
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        icon: <Ico><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /></Ico>,
      },
      {
        id: "debt-analysis", label: "Debt Analysis",
        icon: <Ico><line x1="12" y1="1" x2="12" y2="23" /><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" /></Ico>,
      },
      {
        id: "debt-navigator", label: "Debt Navigator",
        icon: <Ico><polygon points="3 11 22 2 13 21 11 13 3 11" /></Ico>,
      },
      {
        id: "student-loan", label: "Student Loan Aid",
        icon: <Ico><path d="M22 10v6M2 10l10-5 10 5-10 5z" /><path d="M6 12v5c3 3 9 3 12 0v-5" /></Ico>,
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        icon: <Ico><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></Ico>,
      },
      {
...
// ── Disputes page ─────────────────────────────────────────────────────────────
function DisputesPage({ nav }: { nav: (p: PageId) => void }) {
  const active = [
    { creditor: "Midland Credit Management", bureau: "Equifax", type: "COLLECTIONS", status: "PENDING", dateSent: "May 1, 2026", due: "May 31, 2026" },
    { creditor: "Capital One — Auto", bureau: "Experian", type: "LATE_PAYMENT", status: "SENT", dateSent: "May 14, 2026", due: "Jun 13, 2026" },
  ];
  const resolved = [
...
// ── Generic page header ───────────────────────────────────────────────────────
function PageHeader({ eyebrow, title, sub, badge }: { eyebrow: string; title: string; sub: string; badge?: React.ReactNode }) {
  return (
    <div className="cp-page-header">
      <div>
  ...
  return (
    <div>
      ...
      <div className="cp-grid-2">
        <div className="cp-card cp-card-accent-top">
          <div className="cp-card-header">
            <div>
              ...
              <div className="cp-card-subtitle">Active since Jan 1, 2026</div>
            </div>
        ...
        <div className="cp-card">
          <div className="cp-card-header">
            <div><div className="cp-card-title">Payment Method</div></div>
          </div>
          ...
          {[
            ...
            <div key={i} className="cp-flex-between" style={{ padding: "10px 0", borderBottom: "1px solid #eef0f6", fontSize: 13 }}>
              ...
              <span className="cp-pill active">{inv.status}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div className="cp-grid-2">
        <div className="cp-card">
          <div className="cp-card-header">
            <div><div className="cp-card-title">Personal Information</div></div>
            <button className="cp-btn cp-btn-secondary cp-btn-sm">Edit</button>
          </div>
          {[
        ...
        <div className="cp-card">
          <div className="cp-card-header">
            <div><div className="cp-card-title">Case Details</div></div>
          </div>
          {[
            ["Case Type", "Standard Credit Repair"],
            ["Advisor", "ScoreShift AI Team"],
            ["Status", "Active"],
            ["Started", "Jan 1, 2026"],
            ["Plan", "Elite"],
            ["Disputes Filed", "4"],
        ...
        <div className="cp-card">
          ...
          {[
            ...
            <div key={item.key} className="cp-settings-row">
              <div>
                <div className="cp-settings-row-label">{item.label}</div>
                <div className="cp-settings-row-sub">{item.sub}</div>
              </div>
...
function IssuesPage() {
  ...
  return (
    <div>
      <PageHeader eyebrow="Credit Repair" title="Credit Issues" sub="Active negative items and live credit alerts — everything that's changed on your report." badge={<span className="cp-badge live">Live Monitoring</span>} />
      <div className="cp-info-banner">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>
        3 negative items detected across your bureaus. Click "Dispute" to begin the resolution process.
      </div>
      <div className="cp-card">
        <div className="cp-card-header">
        ...
        {issues.map((iss, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 16, padding: "14px 0", borderBottom: i < issues.length - 1 ? "1px solid #eef0f6" : "none" }}>
            ...
            <button className="cp-btn cp-btn-primary cp-btn-sm">Dispute</button>
          </div>
        ))}
      </div>
    </div>
...
export default function AppDemo() {
  ...
  const renderContent = () => {
    switch (displayPage) {
          ...
          <div>
            ...
            <div className="cp-grid-4 cp-mb-24">
              ...
              <div className="cp-stat-card">
                ...
                <div><div className="cp-stat-value">4</div><div className="cp-stat-label">Items Resolved</div></div>
              </div>
              <div className="cp-stat-card">
        ...
        return (
          <div>
            ...
            <ArrayWrapper title="3-Bureau Score History" sub="VantageScore 3.0 — updated with every report pull"
              badge={<span className="cp-badge live">Live</span>} accentTop ready={arrayReady}>
      ...
      // ── Disputes ───────────────────────────────────────────────────────────
      case "disputes":
        return <DisputesPage nav={navigate} />;
        ...
        return (
          <div>
            ...
            <ArrayWrapper title="Identity Monitoring" sub="Dark web scans, breach detection, and real-time alerts"
              badge={<span className="cp-badge live">Active</span>} accentTop ready={arrayReady}>
  ...
  return (
    <div className="cp-portal">
      ...
      <aside className="cp-sidebar">
        <a className="cp-sidebar-logo" href="#" onClick={e => { e.preventDefault(); navigate("dashboard"); }}>
        ...
        <div className="cp-sidebar-user">
          ...
          <div className="cp-user-info">
            ...
            <div className="cp-user-plan">Elite Plan</div>
          </div>
          <button className="cp-signout-btn" title="Sign out">
