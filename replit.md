...
- **UI/UX Decisions**: Animated credit score circles, gamified onboarding, mobile-optimized interfaces, professional layout with 8-tab navigation on `/credit-repair` dashboard. New **Client Portal** at `/portal` (and default for non-admin authenticated users at `/`) — fixed 252px sidebar with 19 nav items across 6 sections, frosted-glass topbar, Mercury dot-grid background, 12 Array web component screens. CSS scoped under `.cp-portal`. Key files: `client/src/pages/client-portal.tsx`, `client/src/styles/portal.css`.
